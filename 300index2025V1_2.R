...
# --- Simple NZ height model (model 2): ha and hb are functions of SI only.
calcheightcoeff_simple <- function(SI, pars = NULL) {
  if (is.null(pars)) pars <- get("coefs", envir = globalenv())
...
CalcMTH_simple <- function(SI, Age, pars = NULL) {
  if (is.null(pars)) pars <- get("coefs", envir = globalenv())
  hc <- calcheightcoeff_simple(SI, pars)
  0.25 + (SI - 0.25) * ((1 - exp(-hc$ha * Age)) / (1 - exp(-hc$ha * 20)))^hc$hb
}

# --- NSW height model (model 1): fixed ha, hb depends on SI.
calcheightcoeff_NSW <- function(SI, pars = NULL) {
  if (is.null(pars)) pars <- get("coefs", envir = globalenv())
  ha <- exp(pars$hNSWa)
...
CalcMTH_NSW <- function(SI, Age, pars = NULL) {
  if (is.null(pars)) pars <- get("coefs", envir = globalenv())
  hc <- calcheightcoeff_NSW(SI, pars)
  0.25 + (SI - 0.25) * ((1 - exp(-hc$ha * Age)) / (1 - exp(-hc$ha * 20)))^hc$hb
}

# --- Height model selector (mirrors VBA heightmod()).
# heightmod_flag: 1=NSW, 2=Simple NZ, 3=Environmental NZ
# latitude: negative for southern hemisphere (NZ range: -48 to -30)
select_heightmod <- function(latitude, elevation) {
  if (heightmod_flag == 1L) return(1L)
  if (is.na(latitude) || is.na(elevation) ||
      latitude < -48 || latitude > -30) return(2L)
  return(3L)
}

# --- Unified CalcMTH dispatcher: selects model based on latitude.
CalcMTH <- function(SI, Age, latitude, elevation, pars = NULL) {
  hmod <- select_heightmod(latitude, elevation)  # needs raw negative lat for NZ range check
  if (hmod == 1L) CalcMTH_NSW(SI, Age, pars)
  else if (hmod == 2L) CalcMTH_simple(SI, Age, pars)
  else CalcMTH_env(SI, Age, abs(latitude), elevation, pars)  # ha formula needs positive lat
}
...
CalcA200start_env <- function(Age, I300, SI, latitude, elevation,
  ...
  MTH30 <- CalcMTH(SI, age300, latitude, elevation, pars)                      # NOTE: normalize at Age=30 for I300 definition
...
solve_SI_from_MTH_env <- function(MTH_obs, Age, latitude, elevation) {
  f <- function(SI) MTH_obs - CalcMTH(SI, Age, latitude, elevation)
...
compute_density_profile <- function(r, ages, SI, I300,
  ...
  mth <- vapply(ages, function(a) CalcMTH(SI, a, lat, elev), numeric(1))
...
build_annual_series_for_plot <- function(r, si_i300_row) {
  ...
  MTH <- vapply(ages, function(a) CalcMTH(SI_est, a, lat, elev), numeric(1))
...
bisection_with_trace <- function(f, lo, hi, iters = 30L, step = 0.05) {
 ...
 show_si_resolution <- function(Age_star, MTH_obs, latitude, elevation,
   ...
   # residual: f(SI) = MTH_obs - CalcMTH(SI, Age*, latitude, elevation)
   f_SI <- function(SI) MTH_obs - CalcMTH(SI, Age_star, latitude, elevation)
   ...
   y0 <- vapply(ages, function(a) CalcMTH(si_seq[1], a, latitude, elevation), numeric(1))
   ...
   if (length(si_seq) > 1) {
     for (k in 2:length(si_seq)) {
       yk <- vapply(ages, function(a) CalcMTH(si_seq[k], a, latitude, elevation), numeric(1))
       lines(ages, yk, col = cols[k], lwd = 2)
     }
   }
   # final curve in black
   y_final <- vapply(ages, function(a) CalcMTH(res$root, a, latitude, elevation), numeric(1))
