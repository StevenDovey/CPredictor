...
voltab <- function() {
# Extract a subset of data for variable 'V' from 'parameters'
V <- as.data.frame(parameters[1:8, 6:16])  # Extract 8 rows and 11 columns starting from column I
...
fn <- function(X, fnno, p1, p2, p3, p4) {
  ...
  if (fnno == 1) {
    I300 <- X
    #assign("I300", I300, envir = .GlobalEnv)  
    Growth(FALSE,I300)  # Dont calculate Growth
    return(DBH300 - DBH)
  } else if (fnno == 2) {
...
for (j in 1:steps) {
  ...
  Age <- Age + steplength
  A200 <- CalcA200start(Age, I300, SI)
  N <- stock(Nelements, nelement, ncum, N, shist, DBH)
   HeightCalc<- (Height(N, nelement, Nelements, Age))
  mnheight <-   HeightCalc$mnheight
  MTH <-HeightCalc$MTH
  adjage <- Ageshifts(N, PRHT, prlag, total_prlag, thin, sellag, Nelements, Age)
  DBH <- Diameter(ncum, dbhelement, Nelements, I300)
  ...
  write.table(iteration_data, "output.csv", sep = ",", row.names = FALSE, col.names = !file.exists("output.csv"), append = TRUE)
  

  
  # Optional: Print step if certain conditions are met
  if (OUTPUT && (abs(Age - floor(Age)) < 0.001 || abs(Age - maxage) < 0.001)) {
    OutStep()
  }
  
  # Thinning and lifting conditions (assuming functions like thinning() and Newlift() are correctly defined)
  if (Stocking_history$shist_N2[shist] != 0 && Age >= Stocking_history$shist_T[shist] - 0.001) {
    thin <- thin + 1
    thinning(N, DBH, shist, Nelements, dbhelement, adjage, thin, totalthinlag, sellag, prlag, I300)
    print(c(thin))
    if (OUTPUT) {
      OutThin()
      OutElements()
  ...
  if (lift < Nlifts && Age >= Pruning_history$lift_T[lift + 1] - 0.001) {
    lift <- lift + 1
    Newlift(lift, Nelements, PRHT, nelement, ncum, total_prlag, prlag, dbhelement, Meanht, adjageel, sellag, MTH, A200, N, SI, pra, prb, prc, Age, totalthinlag, Pruning_history)
    print(c(lift))
    if (OUTPUT) {
      OutPrune()
      OutElements()
  ...
  # Reset lineprinted if necessary
  if (lineprinted) {
...
Process_tree_list <- function(Treelist, Plot_area, Age, nstems) {
  Starting_tree_list <- read_excel("input.xlsx", sheet = "Starting tree list", skip = 5, col_names = TRUE)
  Plot_area <- read_excel("input.xlsx", sheet = "Starting tree list", range = "B3", col_names = FALSE)[[1]]
  Age <- read_excel("input.xlsx", sheet = "Starting tree list", range = "B4", col_names = FALSE)[[1]]
  nstems <- nrow(Starting_tree_list) # Determine the number of stems
  ...
  return(list(MTDia = MTDia, stocking = stocking, petA = petA, petB=petB, qDBH = qDBH, BA = BA))
}
...
stock <- function(Nelements, nelement, ncum,N, shist, DBH) {
  ...
  #print(c(round(N),round(nelement),round(ncum)))
...
Height <- function(N, nelement,Nelements,Age) {
...
#Returns NA thin>0
Ageshifts <- function(N,PRHT, prlag, total_prlag, thin, sellag, Nelements,Age) {
...
Diameter <- function(ncum, dbhelement, Nelements,I300) {
  N <- (ncum[Nelements])
  dbhsqd <- 0
  for (el in 1:Nelements) {
...
thinning <- function(N, DBH,shist,Nelements, dbhelement, adjage, thin, totalthinlag, sellag, prlag,I300) {
  prevN <- N
  prevdbh <- DBH
...
OutputGrowth <- function() {
 ...
 # voltab()      # Call voltab function
  ...
  Growth(OUTPUT)  # Call Growth function
...

{
  ...
  input_data <- suppressMessages(read_excel("input.xlsx", sheet = "Inputs", col_names = FALSE) %>% as.data.frame() )
  Species <-  input_data [2, 4]; assign("Species", Species, envir = .GlobalEnv)  
  input_data <- suppressWarnings(as.data.frame(lapply(input_data, as.numeric)))
  ...
  data_300_index <- suppressMessages(read_excel("input.xlsx", sheet = "300 Index", skip = 0, col_names = FALSE))
  data_300_indexX <- as.data.frame(data_300_index[51:72, 4:6])  # grab the various options marked as an X
  data_300_index <- suppressWarnings(as.data.frame(lapply(data_300_index, as.numeric)))
 ...
 {# Read the 'parameters' sheet from the Excel file
    parameters <- suppressMessages(read_excel("input.xlsx", sheet = "parameters", skip = 0, col_names = TRUE))
    # Assign parameters to the global environment
    assign("parameters", parameters, envir = .GlobalEnv)
    # Extract variable names and values
    variable_names <- parameters$Variable[1:370]  
    variable_values <- (parameters$Coefficients[1:370])
    ...
    for (i in 1:length(variable_names)) {
      ...
      if (!is.na(suppressWarnings(as.numeric(variable_values[i])))) {
        # If numericassign
        assign(variable_names[i], (as.numeric(variable_values[i])), envir = .GlobalEnv)
      } else {
        # If text, assign as character
        assign(variable_names[i], as.character(variable_values[i]), envir = .GlobalEnv)
      }
    }
  }
  # Clean up temporary variables
  rm(variable_names, variable_values, i)
...
print(c(DBH, N, BA, mnheight, Age, I300))
