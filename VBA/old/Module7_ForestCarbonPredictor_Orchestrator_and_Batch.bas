...
'This subroutine predicts carbon using C Change from the yield table in the C Change worksheet
    ...
    Do
        ...
        height2 = Height
        If topheight Then
            If sphaftthin <> 0 Then height2 = MnHt_from_MTH(Height, sphaftthin, MTH_MnHt_a, MTH_MnHt_b)
            Height = MnHt_from_MTH(Height, stocking, MTH_MnHt_a, MTH_MnHt_b)
        End If
        ...
        If sheathdens Then
            dens_sh = density
            dens_wt = (dens_sh * (Volume - (prev_vol - voldead)) + prev_dens_wt * (prev_vol - voldead)) / Volume
        Else
            dens_wt = density
            dens_sh = (dens_wt * Volume - prev_dens_wt * (prev_vol - voldead)) / (Volume - (prev_vol - voldead))
        End If
...
'This suboutine transfers information from "C Change" worksheet to "300 Index" worksheet
    ...
    If latitude <> 0 Then Cells(3, 6) = latitude
