...
Option Explicit
Public Out(1350, 26) As Double 'Declare output matrix for standlevel data
Public Tinit As Double, Ninit As Double, ncal As Double, Tcal As Double, I500 As Double, SBAP As Double


Function test_lookup()
    test_lookup = ReportLookup_dfir(10, T, DBH)
End Function

Function Dfir_Yield()
    Dim thin(3, 5) As Double, prune(3, 6) As Double, i As Integer, j As Integer, N0 As Double, _
        TsM As Double, T As Integer, ic As Integer, th As Integer, BH_Age As Double, Age_BH_ring_30_formed As Double, _
        Density_ring_30 As Double, Adj_Soil_C_N As Double, BH_ring_density As Double, local_parameter As Double, _
        BH_ring_from_pith As Double, Sheath_ratio As Double
'   Worksheets("500 Index").Activate
    I500 = I300
    SI = H30
    N0 = Stock_hist_N(0)
    Adj_Soil_C_N = Soil_C / (Soil_N - 0.14)
    SBAP = Five2SBAP(I500, SI)
    
    For j = 1 To 5
        thin(1, j) = Stock_hist_T(j)
        thin(2, j) = Stock_hist_N(j)
        thin(3, j) = Stock_hist_thin_coeff(j)
    Next j
    For j = 1 To 5
        prune(1, j) = prune_age(i)
        prune(2, j) = prune_height(i)
        prune(3, j) = prune_N(i)
    Next j
    Call Grow(SBAP, SI, N0, rotlength, thin, prune, latitude, 0)
    TsM = Out(0, 0)
    ...
    For T = 1 To rotlength
        ...
        If ic >= 0 Then
            ...
            For th = 1 To 4
                If T >= thin(1, th) And T < thin(1, th) + 1 Then
                    Pre_thin_N(th) = N(T)
                    Pre_thin_dbh(th) = DBH(T)
                    Pre_thin_vol(th) = Volume(BA(T), MTH(T), N(T))
                    N(T) = N(T) - Out(ic + 1, 13)
                    BA(T) = BA(T) - Out(ic + 1, 14)
                    DBH(T) = Sqr(BA(T) / N(T) / PI) * 200
                    Vol(T) = Volume(BA(T), MTH(T), N(T))
                End If
            Next th
        End If
    Next T
    ...
    BH_Age = -(1 / Exp(-3.7082)) * Log(1 - ((1.4 - 0.25) / (SI - 0.25)) ^ (0.3844 + (0.0338 - 0.00057 * _
        (latitude)) * SI) * (1 - Exp(-Exp(-3.7082) * 40)))
    Age_BH_ring_30_formed = 30.5 + BH_Age
    Density_ring_30 = 136.5 + 23.3 * MAT + 3.09 * Adj_Soil_C_N
    local_parameter = (Density_ring_30 - 432.6 - (1.22 - 30) / (0.0235 + 0.0125 * Exp(0.221 * 30))) / _
        (1 - 0.814 * Exp(-30 * 0.258))

    For T = 0 To rotlength
        BH_ring_from_pith = T - BH_Age - 0.5
        If BH_ring_from_pith < 0 Then BH_ring_from_pith = 0
        Sheath_ratio = 1.094 - 0.0277 * Sqr(BH_ring_from_pith)
        BH_ring_density = 432.6 + (1.22 - BH_ring_from_pith) / (0.0235 + 0.0125 * Exp(0.221 * BH_ring_from_pith)) + _
            (1 - 0.814 * Exp(-0.258 * BH_ring_from_pith)) * local_parameter
        WoodDen(T) = Sheath_ratio * BH_ring_density
        Cells(23 + T, 45) = T
        Cells(23 + T, 46) = N(T)
        Cells(23 + T, 47) = DBH(T)
        Cells(23 + T, 48) = BA(T)
        Cells(23 + T, 49) = MTH(T)
        Cells(23 + T, 50) = Vol(T)
        Cells(23 + T, 51) = WoodDen(T)
        Cells(23 + T, 52) = BH_ring_from_pith
        Cells(23 + T, 53) = Sheath_ratio
        Cells(23 + T, 54) = Density_ring_30
        Cells(23 + T, 55) = BH_ring_density
        For th = 1 To 4
            If T <> 0 And T >= thin(1, th) And T < thin(1, th) + 1 Then
                Cells(23 + T, 56) = Pre_thin_N(th)
                Cells(23 + T, 57) = Pre_thin_dbh(th)
                Cells(23 + T, 58) = Pre_thin_vol(th)
            End If
        Next th
    Next T
...
'This function combines the individual components to predict a stands life and stores results in array Out
Public Sub Grow(SBAP As Double, SI As Double, SPHin As Double, Rotation As Double, thin() As Double, prune() As Double, latitude As Double, MA As Double)
    Dim Dummy(1, 6) 'Empty dummy matrix
    Dim SPHs As Variant 'Declare variable to keep track of stocking
    Dim PT, TT, Ut, Ts, dBA, BAb4 As Double 'Variable declaration
'    Dim ReportIndicator As Long 'Thinning time and report indicator
    ...
    Dim ii As Integer, jj As Integer, month_s As Integer, month_f As Integer
    ...
    If SBAP >= 0.2 And SBAP < 5 And SI > 15 And SI < 50 And Rotation > 20 And Rotation <= 100 Then
    ...
    TsM = (TsU + TsD) / 2
    MTH_TsM_Old = 0
    MTH_TsM = MTHcalc(SI, TsM, latitude)
...
'    Ts = TsM 'Set start time
    ...
    MTHs = MTHcalc(SI, Ts, latitude) 'Calculate starting mean top height
...
'        Tf = Ts + 1 / 12 'Next time
        ...
        MTHs = MTHcalc(SI, Tf, latitude) 'Mean top height
        ''''''
...
'        If ReportIndicator = 1 Then
            ...
            Out(ic, 18) = pSPH(0, 4)
            Out(ic, 19) = pSPH(0, 5)
            Out(ic, 20) = Out(ic, 4) - (Out(ic, 15) + Out(ic, 16) + Out(ic, 17) + Out(ic, 18) + Out(ic, 19))
            Out(ic, 21) = Out(ic, 0)
            Out(ic, 22) = Volume(Out(ic, 5), Out(ic, 2), Out(ic, 4))
            Out(ic, 23) = Volume(Out(ic, 14), Out(ic, 2), Out(ic, 13))
            Out(ic, 24) = BA2DBH(Out(ic, 14), Out(ic, 13))
            Out(ic, 25) = CLucalc(Out(ic, 2), Out(ic, 4))
            Out(ic, 26) = Out(ic, 3) - Out(ic, 25)
...
Private Function BAcalc(SBAP, BAs, CL, Ts, Tf, t_last_thin)
    ...
    If (t_last_thin(1, 1) > 0 And dt_thin > 0 And dt_thin < 12) Then
...
Public Function MTHcalc(SI, T, latitude)
    ...
    If PT >= 1 Or TT >= 1 Then
        For i = 1 To 5
            ...
            Else 'If there were no stems in previous step,
                If prune(1, N + 1) <= Ts - 0.1 Then 'then check if it is
...
Sub SBAPsolver()
    ...
    Dim dI500 As Double, mI500 As Double, uI500 As Double, dBA As Double, mba As Double, uba As Double, targetba As Double, _
        start_age As Double, DBH_lookup As Double
    ...
    While Abs(dI500 - uI500) > 0.02
        ...
        start_age = Out(0, 0)
        DBH_lookup = Out(T1 * 12 - start_age * 12, 1)
        
        If DBH_lookup < D1 Then
            dI500 = mI500
        Else
            uI500 = mI500
        End If
...
Sub SIsolver()
    Dim Msg, Style, TITLE, Response, TT, th, uSI, dSI, uH, dH
    TT = T1
    th = H1
    uSI = 50
    dSI = 10
    uH = MTHcalc(uSI, TT, latitude)
    dH = MTHcalc(dSI, TT, latitude)
    While uH - dH > 0.02
...
'This is newly fitted Beekhuiz stand volume function
Public Function BAfromVol(Vol, MTH)
    Dim A, B, D As Double
    If Vol > 0 Then
        A = 0.928
        B = 0.3208
        BAfromVol = Vol / (A + B * MTH)
    Else
        BAfromVol = 0
    End If
End Function
    ...
    For Age = 1 To 20
        ...
        If ddbh >= 2 And prevdbh < 2 Then
            ...
            For j = Age - 1 To 0 Step -1
                Vol(j) = (initialvol + k * j ^ 2.7) * N(j)
                MTH(j) = MTHcalc(SI, j, latitude)
                If MTH(j) < 1.4 Then
                    BA(j) = 0
                Else
                    BA(j) = BAfromVol(Vol(j), MTH(j))
                End If
                DBH(j) = 200 * Sqr((BA(j) / N(j)) / PI)
            Next j
        End If
    Next Age
End Sub
